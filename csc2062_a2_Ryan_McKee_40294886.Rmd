...
Based on the histogram presented seems to have a slightly random distribution although it has 4 clear peaks. In the 0-20  pixels group the first peak this group has around 20 images in this group due to the pixel number being so low I think it is safe to assume that these are the i and ! letters. The next group is 20-50 there is 2 peaks I assume that between 25 to 50 these pixels represent letters assuming this it appears a vast majority of letters have around 30 pixels. after this group there is just random clusters of bars betwen 50-100 pixels I believe that these images as simley faces and sad faces with maybe some thicker letters in there too.
Rows with 1 pixel histogram
This histogram seems to have a right skewed shape with some drops in frequency of some rows with 1 pixel numbers. 
